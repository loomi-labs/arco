      ...
      <div class='flex-1 p-4 pt-0 space-y-2 text-sm'>
        ...
        <div class='flex justify-between items-center'>
          ...
          <div class='flex items-center gap-2'>
            ...
            <span v-if='repo.lastBackup?.status === types.BackupStatus.BackupStatusError' class='tooltip tooltip-top tooltip-error' :data-tip='repo.lastBackup?.message'>
              <ExclamationTriangleIcon class='size-4 text-error cursor-pointer' />
            </span>
            <!-- Warning icon with tooltip -->
            <span v-else-if='repo.lastBackup?.status === types.BackupStatus.BackupStatusWarning && !dismissedWarnings.has(props.repoId)'
                  class='tooltip tooltip-top tooltip-warning' :data-tip='repo.lastBackup?.message'>
