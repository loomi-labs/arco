...
CREATE TABLE `pruning_rules` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `updated_at` datetime NOT NULL, `is_enabled` bool NOT NULL, `keep_hourly` integer NOT NULL, `keep_daily` integer NOT NULL, `keep_weekly` integer NOT NULL, `keep_monthly` integer NOT NULL, `keep_yearly` integer NOT NULL, `keep_within_days` integer NOT NULL, `next_run` datetime NULL, `last_run` datetime NULL, `last_run_status` text NULL, `backup_profile_pruning_rule` integer NOT NULL, CONSTRAINT `pruning_rules_backup_profiles_pruning_rule` FOREIGN KEY (`backup_profile_pruning_rule`) REFERENCES `backup_profiles` (`id`) ON DELETE CASCADE);
...
CREATE TABLE `repositories` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `name` text NOT NULL, `location` text NOT NULL, `password` text NOT NULL, `next_integrity_check` datetime NULL, `stats_total_chunks` integer NOT NULL DEFAULT (0), `stats_total_size` integer NOT NULL DEFAULT (0), `stats_total_csize` integer NOT NULL DEFAULT (0), `stats_total_unique_chunks` integer NOT NULL DEFAULT (0), `stats_unique_size` integer NOT NULL DEFAULT (0), `stats_unique_csize` integer NOT NULL DEFAULT (0));
...
CREATE TABLE `settings` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `theme` text NOT NULL DEFAULT ('system'));
