...
CREATE TABLE `new_failed_backup_runs` (`id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `error` text NOT NULL, `failed_backup_run_backup_profile` integer NOT NULL, `failed_backup_run_repository` integer NOT NULL, CONSTRAINT `failed_backup_runs_backup_profiles_backup_profile` FOREIGN KEY (`failed_backup_run_backup_profile`) REFERENCES `backup_profiles` (`id`) ON DELETE CASCADE, CONSTRAINT `failed_backup_runs_repositories_repository` FOREIGN KEY (`failed_backup_run_repository`) REFERENCES `repositories` (`id`) ON DELETE CASCADE);
